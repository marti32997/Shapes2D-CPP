...
Rectangle::Rectangle(const std::array<Point, 4> &vertices, const bool &warp_space)
	: Shape2D("Rectangle", warp_space), vertices_(vertices)
{
	area_ = compute_Area();
...
{
	int length = vertices_[2].get_X() - vertices_[0].get_X();
	int width = vertices_[0].get_Y() - vertices_[1].get_Y();
	return abs(length) * abs(width);
}

bool Rectangle::is_Point_In_Shape(const int &x, const int &y) const
{
...
bool Rectangle::is_Point_On_Shape(const int &x, const int &y) const
{
...
const Point &Rectangle::get_Vertex_0() const { return this->vertices_[0]; }
const Point &Rectangle::get_Vertex_1() const { return this->vertices_[1]; }
const Point &Rectangle::get_Vertex_2() const { return this->vertices_[2]; }
const Point &Rectangle::get_Vertex_3() const { return this->vertices_[3]; }
